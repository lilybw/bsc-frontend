...
const VideoFrame: Component<VideoDemoFrameStyleProps> = (props) => {
    const computedStyle = createMemo(
        () => css`
            ${videoFrameBackgroundStyle} ${props.styleOverwrite}
        `,
    );
        ...
        <div>
            <div class={computedStyle()}>
                <NTAwait func={() => props.backend.assets.getMetadata(1010)}>
                    {(asset) => <GraphicalAsset styleOverwrite={videoIconStyleOverwrite} metadata={asset} backend={props.backend} />}
                </NTAwait>
                {props.children}
            </div>
        </div>
...
const videoFrameBackgroundStyle = css`
    ...
    backdrop-filter: blur(0.5rem) drop-shadow(0 0 0.5rem black);
    -webkit-backdrop-filter: blur(0.5rem) drop-shadow(0 0 0.5rem black); // For Safari support
