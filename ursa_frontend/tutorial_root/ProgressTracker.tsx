...
export default function ProgressTracker(props: ProgressTrackerProps): JSX.Element {
    ...
    return (
        <div
            class={css`
                ${progressTrackerStyle} ${props.styleOverwrite}
            `}
            id="tutorial-progress-tracker"
        >
            <div
            ...
            />
            <For each={props.slideStore}>
                {(elem, index) => (
                    <div class={iconContainerStyle}>
                        <NTAwait func={() => props.backend.assets.getMetadata(elem.iconId)}>
                            {(asset) => <GraphicalAsset styleOverwrite={iconStyleOverwrite} metadata={asset} backend={props.backend} />}
                        </NTAwait>
                    </div>
                )}
            </For>
