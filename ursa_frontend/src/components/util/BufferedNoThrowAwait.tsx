...
const BNTAwait = <T, R extends Component>(props: NoThrowAwaitProps<T, R>) => {
    ...
    return (
        <>
            ...
            <Show when={primaryResource.error}>{getSecondaryOrFallback()}</Show>
            <Show when={primaryResource.latest}>
                <Show when={primaryResource.latest?.err !== null}>{getSecondaryOrFallback()}</Show>
                <Show when={primaryResource.latest?.res !== null}>{children(primaryResource.latest!.res!)}</Show>
            </Show>
        </>
    );
};
