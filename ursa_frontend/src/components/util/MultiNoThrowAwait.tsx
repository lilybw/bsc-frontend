...
const MNTAwait = <T extends any[]>(props: NoThrowAwaitProps<T>) => {
    ...
    const onFallback = (error: Error | Error[] | any) => {
        return <>{fallback ? fallback(JSON.stringify(error)) : <SomethingWentWrongIcon message={JSON.stringify(error)} />}</>;
    };

    return (
        <>
            <Show when={resource.loading}>{props.whilestLoading ? props.whilestLoading : <Spinner />}</Show>
            <Show when={resource.error}>{onFallback(resource.error)}</Show>
