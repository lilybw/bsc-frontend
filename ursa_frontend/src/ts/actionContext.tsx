...
export const ActionContextIcon = (assetId: number): Component<ActionContextIconProps> => {
    return (props) => {
        return (
            <NTAwait func={() => props.backend.assets.getMetadata(assetId)}>
                {(asset) => <GraphicalAsset styleOverwrite={imgStyleOverwrite} backend={props.backend} metadata={asset} />}
            </NTAwait>
        );
    };
};
