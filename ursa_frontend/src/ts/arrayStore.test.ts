...
describe('ArrayStore', () => {
  ...
  describe('findFirst', () => {
    ...
    it('should find the first element amount duplicates', () => {
      const store = createArrayStore<{ id: Number; value: number }>([{ id: 1, value: 1 }, { id: 2, value: 2 }, { id: 3, value: 2 }]);
    ...
    it('should work with mutations', () => {
      ...
      const found = store.findFirst((el) => el.value === 2);
      expect(found).toBeUndefined();
      store.mutateElement(0, e => { e.value = 2; return e;});
      const found2 = store.findFirst((el) => el.value === 2);
      expect(found2).not.toBeUndefined();
  ...
  describe('findAll', () => {
    ...
    it('should work with mutations', () => {
      ...
      store.mutateElement(0, e => { e.id = 2; return e;});
  ...
  describe('mutateElement', () => {
    ...
    it('succeeds, in place mutation', () => {
      ...
      const mutated2 = store2.mutateElement(0, (el) => {el.id = 3; return el;}); // in place mutation
    ...
    it('should mutate an element ("in place")', () => {
      ...
      const element = { id: 1, value: 'old' };
      store.add(element);
      store.mutateElement(0, (el) => { el.value = 'new'; return el; });
      expect(store.get).toEqual([{ id: 1, value: 'new' }]);
    });
    ...
    it('in-place mutateElement should be reactive', {timeout: 500}, () => {
      return testEffect((dispose) => {
        ...
        createEffect(() => {
          ...
          const mutated = store.mutateElement(0, (el) => {el.id += 1; return el;});
